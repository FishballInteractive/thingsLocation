...
ifeq ($(DEBUG), 1)
CCFLAGS += -O0 -s ASSERTIONS=1 -s SAFE_HEAP=1 -s GL_UNSAFE_OPTS=0
CXXFLAGS += -O0 -s ASSERTIONS=1 -s SAFE_HEAP=1 -s GL_UNSAFE_OPTS=0
DEFINES += -D_DEBUG -DCOCOS2D_DEBUG=1 -DCP_USE_DOUBLES=0
OBJ_DIR := $(OBJ_DIR)/debug
LIB_DIR := $(LIB_DIR)/debug
BIN_DIR := $(BIN_DIR)/debug
else
CCFLAGS += -O1  -s GL_UNSAFE_OPTS=0 -s ASM_JS=1
CXXFLAGS += -O1 -s GL_UNSAFE_OPTS=0 -s ASM_JS=1
